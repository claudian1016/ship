...
class ShipEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.rewards_weather_matrix = RandomState(1234567890).randint(-2,1, size = (self.size+2, self.size+2))  
        ...
        self.observation_space = spaces.Box(np.array([0,0]), np.array([self.size +3, self.size +3]), dtype = int)        
        #print(self.observation_space)
        
        # reward
        self.reward_range = (-100, 100)

    #take out functions from step and call them within (for cleaner code)
    def step(self, action):

        #action = self.actions
        self.nrow, self.ncol = nrow, ncol = self.desc.shape
        
        #changing from numbered states, do not make 
        
        #def to_state(self, newrow, newcol, oldrow, oldcol):    
        def to_state(self, newrow, newcol):    
        ...
        #method for movement --> changing states
